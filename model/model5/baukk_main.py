                ...
                if prev_x is not None and prev_y is not None:
                    ...
                    if abs(dx) > abs(dy):
                        if dx > 50:  # right
                            ...
                            cv2.putText(frame, "Action Right", (cx_0, cy_0), cv2.FONT_HERSHEY_DUPLEX,            
                                1, (212,130,173), 1, cv2.LINE_AA)
                        elif dx < -50:  # left
                            # pyautogui.press('left')
                            cv2.putText(frame, "Action Left", (cx_0, cy_0), cv2.FONT_HERSHEY_DUPLEX,            
                                1, (212,130,173), 1, cv2.LINE_AA)
                    else:  # Vertical swipe
                        if dy > 50:  # down
                            # pyautogui.press('down')
                                         cv2.putText(frame, "Action Down", (cx_0, cy_0), cv2.FONT_HERSHEY_DUPLEX,            
                                1, (212,130,173), 1, cv2.LINE_AA)
                        elif dy < -50:  # up
                            # pyautogui.press('up')
                                         cv2.putText(frame, "Shitted Myself", (cx_0, cy_0), cv2.FONT_HERSHEY_DUPLEX,            
                                1, (212,130,173), 1, cv2.LINE_AA)
